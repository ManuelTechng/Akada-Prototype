  ...
  useEffect(() => {
    const fetchPrograms = async () => {
      ...
      try {
        ...
        if (cachedPrograms && cachedPrograms !== "[object Object]") {
          try {
            const parsedPrograms = JSON.parse(cachedPrograms);
            setPrograms(parsedPrograms);
            setLoading(false);
            return;
          } catch (parseError) {
            console.warn('Failed to parse cached programs, removing invalid cache:', parseError);
            localStorage.removeItem('akada_programs');
          }
        }
