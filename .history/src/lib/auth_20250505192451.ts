...
export const getUserProfile = async (userId: string): Promise<UserProfile | null> => {
  try {
    ...
    if (!response.ok) {
      ...
      console.error('Error creating initial profile:', response.status, errorText);
      throw new Error(`Failed to create initial profile: ${response.status} ${errorText}`);
    }
    
    const result = await response.json();
    console.log('Initial profile created successfully:', result);
    
    // Return the newly created profile
    return Array.isArray(result) && result.length > 0 ? result[0] : newProfile;
